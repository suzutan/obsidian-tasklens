...
- **Preact + JSX**（Reactではない）— `jsxImportSource: "preact"`, `react→preact/compat`エイリアス
- **@preact/signals** — リアクティブ状態管理
- **Vite+ (vp)** — ビルド（Rolldown）・テスト（Vitest）・lint（Oxlint）・fmt（Oxfmt）統合ツールチェーン
...
- **テスト**: `npm test`（vp test run）, `npm run test:watch`（vp test）
- **リント/フォーマット**:
  - `npm run format` — Oxfmt 実行（`src/` を自動修正）
  - `npm run format:check` — Oxfmt チェック（CI用）
  - `npm run lint` — Oxlint 実行（`typescript/no-explicit-any: error`）
  - `npm run check` — format + lint + typecheck 一括実行
- **プリコミットフック**: `vp staged`（vite.config.ts 内で設定、`vp check --fix` を実行）
- 成果物: `main.js`（バンドル）, `styles.css`（手書き）, `manifest.json`
- `styles.css` はObsidian CSSカスタムプロパティ（`--text-normal`, `--background-primary`等）を使用
- 設定は `vite.config.ts` に統合（ビルド・テスト・lint・fmt・staged すべて）
- 外部モジュール: `obsidian`, `electron`, `@codemirror/*`, `@lezer/*`, Node.js built-ins
