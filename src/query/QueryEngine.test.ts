...
describe("executeQuery", () => {
  const tasks: Task[] = [
    ...
    makeTask({ content: "タスク2", completed: true, dueDate: "2026-03-10", priority: 4, doneDate: "2026-03-10", order: 1 }),
    ...
    makeTask({ content: "タスク5", completed: false, scheduledDate: "2026-03-10", labels: ["仕事"], recurrence: { type: "weekly", interval: 1 }, order: 4 }),
  ];
  ...
  it("filters by due today", () => {
    const query: ParsedQuery = { filter: { type: "date_filter", field: "due", op: "today", value: "" }, sort: [], group: [], limit: null };
    const result = executeQuery(tasks, query);
    expect(result.length).toBe(1);
    expect(result[0].content).toBe("タスク2");
  });
  ...
  it("filters by due before today (overdue)", () => {
    const query: ParsedQuery = { filter: { type: "date_filter", field: "due", op: "before_today", value: "" }, sort: [], group: [], limit: null };
    const result = executeQuery(tasks, query);
    expect(result.length).toBe(1);
  ...
  it("filters by description includes", () => {
    const query: ParsedQuery = { filter: { type: "description_includes", text: "タスク1" }, sort: [], group: [], limit: null };
    const result = executeQuery(tasks, query);
    expect(result.length).toBe(1);
...
describe("groupTasks", () => {
  const tasks: Task[] = [
    makeTask({ content: "タスク1", projectPath: "project-a.md", dueDate: "2026-03-10", priority: 1, labels: ["仕事"], section: "Section A", order: 0 }),
    makeTask({ content: "タスク2", projectPath: "project-a.md", dueDate: "2026-03-10", priority: 4, labels: ["家事"], section: "Section B", order: 1 }),
    makeTask({ content: "タスク3", projectPath: "project-b.md", priority: 2, labels: ["仕事"], section: "Section A", order: 2 }),
  ];
